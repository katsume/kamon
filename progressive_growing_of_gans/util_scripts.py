...
def generate_interpolation_video(run_id, snapshot=None, grid_size=[1,1], num_mp4s=1, image_shrink=1, image_zoom=1, duration_sec=60.0, smoothing_sec=1.0, mp4_prefix=None, mp4_fps=30, mp4_codec='libx265', mp4_bitrate='16M', random_seed=1000, minibatch_size=8):
    network_pkl = misc.locate_network_pkl(run_id, snapshot)
    if mp4_prefix is None:
        mp4_prefix = misc.get_id_string_for_network_pkl(network_pkl) + '-lerp'
    ...
    result_subdir = misc.create_result_subdir(config.result_dir, config.desc)
    for mp4_idx in range(num_mp4s):
        print('Generating latent vectors...')
        all_latents = random_state.randn(*shape).astype(np.float32)
        all_latents = scipy.ndimage.gaussian_filter(all_latents, [smoothing_sec * mp4_fps] + [0] * len(Gs.input_shape), mode='wrap')
        all_latents /= np.sqrt(np.mean(np.square(all_latents)))
        print('Generating mov %d / %d...' % (mp4_idx, num_mp4s))
        moviepy.editor.VideoClip(make_frame, duration=duration_sec).write_videofile(os.path.join(result_subdir, '%s%06d.mp4'%(mp4_prefix, mp4_idx)), fps=mp4_fps, codec='libx264', bitrate=mp4_bitrate)
    open(os.path.join(result_subdir, '_done.txt'), 'wt').close()
